<template>
  <div class="create-product-outer">
    <div class="create-product-container">
      <h1 class="create-product-title">Atualizar Produto</h1>
      <form class="create-product-form" @submit.prevent="handleSubmit">
        <div class="input-group category-group">
          ...
          <button type="button" class="add-category-btn" @click="showCategoryModal = true" title="Adicionar categoria">+</button>
        </div>
        ...
        <div class="input-group">
          <input v-model.number="form.price" type="number" class="input" placeholder="Preço" min="0" step="0.01" required />
        </div>

        <div class="input-group">
          <label>Imagem atual:</label>
          <img v-if="existingImageUrl" :src="existingImageUrl" alt="Imagem do produto" style="max-width: 150px; margin-bottom: 8px;" />
          <input type="file" accept="image/*" @change="onImageChange" class="input" />
        </div>
        ...
        <div class="input-group">
          <label>Arquivo STL atual:</label>
          <a v-if="existingModelUrl" :href="existingModelUrl" target="_blank" rel="noopener" style="display:block; margin-bottom: 8px;">
            Ver arquivo STL
          </a>
          <input type="file" accept=".stl" @change="onModelChange" class="input" />
        </div>

        <div class="input-group">
          <textarea v-model="form.description" class="input" placeholder="Descrição" rows="4" required></textarea>
        </div>
...
const existingModelUrl = ref(null)
...
async function loadProductById(id) {
  try {
    ...
    // Colocar a url da imagem principal para mostrar no preview
    if (data.productImages && data.productImages.length > 0) {
      const mainImage = data.productImages.find(img => img.type === 'main')
      if (mainImage) existingImageUrl.value = mainImage.url
    }

    // Colocar a url do arquivo STL
    if (data.fileUrl) existingModelUrl.value = data.fileUrl
  } catch {
...
function onImageChange(e) {
  ...
  // Atualiza preview com o arquivo novo
  existingImageUrl.value = URL.createObjectURL(file)
}
...
function onModelChange(e) {
  ...
  if (!file) return
  form.model = file
  // Não temos preview do STL, mas poderíamos armazenar algo aqui se quiser
}
...
async function handleSubmit() {
  ...
  try {
    ...
    // Se o usuário selecionou uma nova imagem, envia, senão não
    if (form.images[0].image) {
      formData.append('images[0].type', 'main')
      formData.append('images[0].image', form.images[0].image)
    }

    // Se selecionou novo modelo, envia, senão não
    if (form.model) {
...
onMounted(async () => {
  ...
  if (route.params.id) {
    await loadProductById(route.params.id)
  } else {
    errorMessage.value = 'ID do produto não informado.'
  }
})
</script>
